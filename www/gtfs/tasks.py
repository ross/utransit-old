...
class CreateStopArrivals(object):
    ...
    def run(self, stop_id):
        cursor = connection.cursor()

        active_service_ids = ('SUN')

        # TODO: schedule aware
        cursor.execute('''select t.id, t.route_id, t.direction_id,
                       st.arrival_time, st.departure_time
                       from gtfs_stoptime st join
                       gtfs_trip t on st.trip_id = t.id
                       where st.stop_id = %s and t.service_id in (%s)
                       order by st.arrival_time''',
                       (stop_id, active_service_ids))
